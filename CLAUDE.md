...
How a `Qcow2Dev` is created and used (spans utils.rs + src/dev/):
...
- **Core `Qcow2Dev` methods** (src/dev/mod.rs + IO submodules): `read_at(buf, offset)`, `write_at(buf, offset)`, `discard(offset, len)`, `flush_meta()` (lazy soft-update flush of dirty metadata), `fsync_range(off, len)`, `get_mapping(virt_offset)`, `check()` (integrity + leak check), and `qcow2_cluster_usage()` for stats.
